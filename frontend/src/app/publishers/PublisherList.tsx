...
const ROUTE = "publisher-list";

const LIST_PUBLISHERS = gql(/* GraphQL */ `
...
const DELETE_PUBLISHER = gql(/* GraphQL */ `
  mutation deletePublisher($id: String!) {
    deletePublisher(id: $id)
  }
`);

export const PublisherList = observer(({ onSelect }: EntityListScreenProps) => {
  const screens: Screens = useScreens();
  const intl = useIntl();
  const match = useRouteMatch<{ entityId: string }>(`/${ROUTE}/:entityId`);
  const history = useHistory();

  const { loading, error, data } = useQuery(LIST_PUBLISHERS);

  const [executeDeleteMutation] = useMutation(DELETE_PUBLISHER);
  ...
  const items = data?.listPublishers;
...
function getUpdateFn(e: any) {
  return (cache: ApolloCache<any>) => {
    cache.modify({
      fields: {
        listPublishers(existingRefs, { readField }) {
