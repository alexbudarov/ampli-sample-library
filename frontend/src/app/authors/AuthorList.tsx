...
const LIST_AUTHORS = gql(/* GraphQL */ `
  query listAuthors {
    listAuthors {
      email
      firstName
...
const DELETE_AUTHOR = gql(/* GraphQL */ `
  mutation deleteAuthor($id: Long!) {
    deleteAuthor(id: $id)
  }
`);

export const AuthorList = observer(({ onSelect }: EntityListScreenProps) => {
  const screens: Screens = useScreens();
  const intl = useIntl();
  const match = useRouteMatch<{ entityId: string }>(`/${ROUTE}/:entityId`);
  const history = useHistory();

  const { loading, error, data } = useQuery(LIST_AUTHORS);

  const [executeDeleteMutation] = useMutation(DELETE_AUTHOR);
  ...
  const openEditor = useCallback(
    (id?: string) => {
      const params: OpenInBreadcrumbParams = {
        breadcrumbCaption: intl.formatMessage({ id: "screen.AuthorDetails" }),
...
function getUpdateFn(e: any) {
  return (cache: ApolloCache<any>) => {
    cache.modify({
      fields: {
        listAuthors(existingRefs, { readField }) {
